...
const getBoards = (data: string[]) => data.slice(1).map((board, i): Board => ({
    id: i,
    values: [
        ...board.split('\n')
            .filter(str => str.trim())
...
const markCalled = (call: string) => ({ id, values }: Board): Board => {
    return {
        id,
...
const markWinner = (call: string) => ([isWinner, board]: [boolean, Board]): void => {
    if(isWinner) {
        winners.set(board.id, { board, call });
    }
}

const sumUnmarked = ({ values }: Board) => values.reduce((sum, row) => {
    sum += row.filter(space => space !== 'X')
        .reduce((sum, curr) => {
...
for(let call of calls) {
    ...
    boards
        .filter(filterWinningBoards(winners))
        .forEach(processBoard);
